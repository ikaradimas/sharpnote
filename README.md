...
- **Export as App (Beta)** — File → Export as App (Beta)… packages the current notebook as a standalone Electron app (~350 MB) that opens directly to the notebook with full fidelity; supports macOS (.app bundle) and Windows (.exe); the exported app auto-loads the embedded notebook on launch
