...
- **Dependency-first execution** — running a code cell first walks its dependency tree and runs any *stale previous* dependencies (topological order) before the cell itself; a dependency is any cell that produces a variable this cell consumes, plus any explicitly-wired links. Only cells positioned **before** the target run (a variable reassigned lower down never drags the next cell in), and dependencies that already ran successfully and are unchanged are skipped. Cells with no data-flow and no explicit links default to **no dependencies** (run alone) — there is no implicit notebook-order chaining
- **Manual-only cells** — mark a runnable cell "manual only" (hand icon in its header) to exclude it from every automatic run: it is never pulled in as another cell's dependency, is skipped by Run All / Run from / Run to, traversal does not pass through it, and it won't auto-fire from a schedule, presentation mode, or run-on-startup — it runs only from its own ▶. Purpose-built for side-effecting cells (SQL writes, HTTP POSTs, shell commands) so running a downstream cell can't silently re-fire them (explicit Orchestration-panel Run-downstream/pipeline actions still include them)
- **Ambiguous-producer warning + run-plan preview** — when two cells declare the same variable ("last writer wins" picks the edge), both are badged with the shared variable and the other producers so the choice is visible; hovering a cell's ▶ shows a tooltip of exactly which cells will also run (its stale dependencies) and any manual-only producers skipped
- **Reactive Cell Dependencies** — after a successful execution, cells that are downstream in the dependency graph of the changed variable (data-flow consumers plus cascading dependents) are flagged with a "↺ upstream variables changed" banner, clearing automatically when those cells are run
...
- **Docker/Floci lifecycle** — Starting… and Stopping… intermediate states with pulsing badge; already-running container detection reuses existing containers instead of failing; Run All now executes all cell types
...
- **Rich output** — `Display.Html()`, `Display.Table()`, `Display.Graph()` (Chart.js), `Display.Csv()`, `Display.Image()` (URL / file path / base64 data URI), and `Console.Write` captured as `stdout`; very large tables are capped to the first 50,000 rows (with a "showing first 50,000 of N" notice) so a huge result never freezes the app
...
- **Export as App (Beta)** — File → Export as App (Beta)… packages the current notebook as a standalone Electron app (~350 MB) that opens directly to the notebook with full fidelity; supports macOS (.app bundle) and Windows (.exe); optional passphrase protection (PBKDF2 + AES-256-GCM) and credential stripping with viewer-side prompts for missing secrets
...
- **Parameterised notebooks** — declare top-of-notebook typed inputs in the `.cnb` `params` array (`double`/`int`/`string`/`bool`/`choice`); they render as a compact form above the cells and are injected as typed locals into every code cell so user code writes `if (Threshold > 0.5)` directly; CLI override via `sharpnote run report.cnb --param Threshold=0.9` for cron / CI
- **Output snapshots** — toggle the camera button on any cell to capture its outputs to `<notebook-dir>/.snapshots/<cell-id>.snap.json`; subsequent runs compare and badge the cell ✓ / ✗; `sharpnote run --check-snapshots` exits 3 if any snapshot drifted, distinct from runtime-error exit 1
- **Profile panel** — sortable per-cell timing table with last / average / total / run count and a sparkline of the last 20 executions; click a row to jump to that cell
- **PDF export** — File → Export as PDF… renders the current notebook to A4 via Electron's `printToPDF`, with a dedicated `@media print` stylesheet that hides chrome (tabs, toolbar, panels, cell controls) and neutralises the dark theme for ink-friendly output; cells get `break-inside: avoid` so they stay together where possible
- **Marp slide decks in markdown cells** — when a markdown cell (or `Display.Markdown` output) opens with `---\nmarp: true\n---` frontmatter, the renderer switches to a paginated slide view via `@marp-team/marp-core` with a bottom-center toolbar (prev / next / fullscreen) and arrow-key / PageUp / PageDown navigation; slide titles are excluded from the Table of Contents; the Display & Rich Output template ships with a working sample deck
...
- **Bounded memory on long runs** — re-running a cell no longer retains the previous run's data: after every execution the kernel prunes shadowed variable bindings (the older copies a re-declaration leaves behind) so only the current values stay live. Per-cell output is capped (excess coalesced into a single "N earlier outputs hidden to conserve memory" marker) and the live log buffer retains only recent entries; the kernel's cross-cell analysis workspace de-duplicates re-run cells rather than growing unbounded; the kernel runs with workstation GC + `RetainVM=false` + `GC.ConserveMemory` so freed memory returns to the OS. When kernel memory climbs past a threshold the status bar shows a warning you can click to restart the kernel (a fresh process that fully reclaims accumulated memory); to free a single large variable without restarting, use the inspector's Free button or set it to `null` in a cell
...
- **Per-cell variable inspector** — each code cell header has an inspector button listing the variables that cell declares; choosing one opens a draggable, resizable, persistent popup that renders the value using the same inference as `.Display()` (collection → table, object → tree, scalar → text) and re-renders after each cell execution while in scope (the value is re-fetched from the kernel, so large collections stay current); each popup has an eraser (Free) button that sets the variable to null on the kernel to release its memory without a restart
...
- **Log panel** — structured, time-stamped kernel log stream with `NOTEBOOK` lifecycle entries and `USER` entries written by `.Log()` calls in scripts; loading a saved log file shows only its last 1,000 entries (so a large file never freezes the panel), with an "Open full file" link to the complete log — the live stream is never truncated this way
...
- **Table of Contents** — live heading outline from markdown cells; click any entry to scroll to it; headings are colour- and weight-coded by level (H1 primary accent, H2 secondary accent, H3 dimmer) from the active theme's palette, so structure is easy to scan and headings read distinctly from the neutral code-cell entries
...
- **Embedded Files panel** — store files inline in the notebook and read them from code via `Files["name"]` (`Ctrl+Shift+E`); import a file from disk with the + button, expand a file to preview the first 100 lines of its contents (binary files show a note instead), and download any file back to disk with the export button (binary-safe — base64 entries decode to their original bytes); edit metadata and per-file variables inline. Colliding import names are auto-suffixed (`name_2`, `name_3`, …)
...
- **Visual query builder** — the Database panel includes a visual query builder for constructing SELECT queries with table joins and filters; connection health pulse shows live connectivity status; each table row in the schema tree has a clipboard badge that copies its generated C# type name
...
- **Database integration** — connect to SQLite, SQLite (In-Memory), SQL Server, PostgreSQL, or Redis; for relational providers the schema is introspected and a typed `DbContext` + POCO classes are code-generated and injected; each table's POCO also gets a best-effort EF-style singular alias (a `Purchases` table → both `Purchases` and `Purchase`, the same type) usable at runtime and in IntelliSense; Redis injects a `StackExchange.Redis.IDatabase` variable
...
| `VarsPanel` | Live variable snapshot from the kernel (name, type, value) with diff highlights |
