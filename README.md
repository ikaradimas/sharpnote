...
- **Cell Output History** — re-running a cell preserves the previous outputs; a ‹ › navigator in the cell footer lets you browse the last 5 runs to compare results across executions; pin a historical output for side-by-side comparison with the current output
...
- **Output Pinning** — pin any historical output using the 📌 button in the history navigator; a side-by-side comparison view shows the pinned output alongside the current output for easy regression checking
