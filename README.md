...
- **Bounded memory on long runs** — per-cell output is capped (excess coalesced into a single "N earlier outputs hidden to conserve memory" marker) and the live log buffer retains only recent entries; the kernel's cross-cell analysis workspace de-duplicates re-run cells rather than growing unbounded; the kernel runs with workstation GC + `RetainVM=false` + `GC.ConserveMemory` so freed memory returns to the OS. When kernel memory climbs past a threshold the status bar shows a warning you can click to restart the kernel (a fresh process that fully reclaims accumulated memory); to free a single large variable without restarting, set it to `null` in a cell
