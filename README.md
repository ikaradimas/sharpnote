...
- **Bounded memory on long runs** — per-cell output is capped (excess coalesced into a single "N earlier outputs hidden to conserve memory" marker) and the live log buffer retains only recent entries; the kernel's cross-cell analysis workspace de-duplicates re-run cells rather than growing unbounded; Run → Reset Kernel drops accumulated script state to reclaim most kernel memory (a full reclaim needs a kernel restart)
