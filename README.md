...
- **Dependency-first execution** — running a code cell first walks its dependency tree and runs its dependencies (topological order) before the cell itself; a dependency is any cell that produces a variable this cell consumes, plus any explicitly-wired links. Cells with no data-flow and no explicit links default to **no dependencies** (run alone) — there is no implicit notebook-order chaining
