...
export function useNotebookManager({ cancelPendingCellsRef, saveSettingsRef, formatOnSaveRef }) {
  ...
  const buildNotebookData = useCallback((notebookId) => {
    ...
    return {
      ...
      cells: nb.cells.map(({ id, type, content, name, color, columns, nextCells, prevCells, outputMode, locked, codeFolded, presenting, presentInterval, snapshot, db, label, mode, truePath, falsePath, switchPaths, image, containerName, ports, env, volume, command, runOnStartup, runOnShutdown }) => ({
        id, type, content,
        ...(name ? { name } : {}),
        ...(color ? { color } : {}),
        ...(columns ? { columns } : {}),
