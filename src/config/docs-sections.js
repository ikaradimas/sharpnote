...
export const DOCS_SECTIONS = [
  ...
  },
  {
    ...
    content: [
      ...
      { type: 'p', text: 'A global Util object provides LinqPAD-compatible utility methods:' },
      { type: 'ul', items: [
        'obj.Dump() — alias for obj.Display(); renders the object in the output area',
        'list.DumpTable() — alias for list.DisplayTable(); renders as a data table',
        'Util.Cmd(command, args?, workingDir?) — run a shell command and display its output',
        'Util.Time(action, label?) — benchmark an Action and display elapsed time',
        'Util.Time<T>(fn, label?) — benchmark a Func<T>, display timing, and return the result',
        'Util.Dif(a, b, labelA?, labelB?) — show a line-by-line diff between two values',
        'Util.HorizontalRun(separator, items…) — display multiple objects side by side',
        'Util.Metatext(text) — display dimmed gray metadata text',
        'Util.Highlight(obj, color?) — display an object inside a colored highlight box',
        'Util.Cache<T>(key, getValue) — memoize a computation across executions until kernel reset',
