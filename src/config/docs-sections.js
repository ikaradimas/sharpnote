...
export const DOCS_SECTIONS = [
  ...
  },
  {
    ...
    content: [
      ...
      { type: 'p', text: 'To keep this in check: per-cell output is capped (if a cell emits thousands of lines or displays, the oldest are coalesced into a single "N earlier outputs hidden to conserve memory" marker), and the Log panel\'s live buffer keeps only the most recent entries. Run → Reset Kernel drops the accumulated script state and reclaims most of this memory; the assemblies compiled during the session stay loaded until the kernel process itself restarts, so for a very long session, restarting the kernel reclaims the rest.' },
      { type: 'h3', text: 'Resetting the Kernel' },
      { type: 'p', text: 'Run → Reset Kernel sends a reset command, clearing the accumulated script state: variables, the cross-cell analysis source, and using directives. Cell content and output are preserved. Attached DB contexts, NuGet packages, and Config values are re-injected automatically after reset. Note that assemblies compiled or loaded during the session remain in the process (a fresh kernel process is needed to unload them), so reset frees most — not all — of the memory a long session accumulates.' },
