...
export const DOCS_SECTIONS = [
  ...
  },
  {
    ...
    content: [
      ...
      { type: 'p', text: 'The kernel keeps every variable and all compiled state from prior runs so later cells can build on them — this is what makes the notebook feel like one continuous script. A consequence is that re-running a cell that holds large data (e.g. a big CSV loaded into a list) keeps the previous run\'s data in memory too, until you reset. Memory therefore grows across a long editing session, especially when repeatedly re-running heavy cells.' },
      { type: 'p', text: 'Several things keep this in check. Per-cell output is capped (if a cell emits thousands of lines or displays, the oldest are coalesced into a single "N earlier outputs hidden to conserve memory" marker), and the Log panel\'s live buffer keeps only the most recent entries. When the kernel\'s memory climbs past a threshold, the status bar shows a red warning; clicking it restarts the kernel (see below). The runtime is also tuned to return freed memory to the OS, so working-set falls back after a cell\'s large intermediate results are garbage-collected.' },
      { type: 'p', text: 'To free a single large variable without restarting, click the eraser (Free) button in its inspector popup, or just assign it null in a cell — e.g. bigData = null; — which drops that variable\'s current value (if nothing else references it). Note this frees the current binding; the shadowed copies left by re-running the cell that declared it are only cleared by restarting the kernel.' },
  ...
  },
  {
    ...
    content: [
      ...
      { type: 'p', text: 'Each popup also has an eraser (Free) button next to Close. Clicking it frees that variable — the kernel sets it to null so the object it held can be garbage-collected — to release memory without restarting the kernel. Use it for large variables (big collections, DataTables, byte buffers) you are done with; you can always re-run the declaring cell to recompute the value. Free is shown only while the variable is in scope and not already null.' },
    ],
  },
  {
