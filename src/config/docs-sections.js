...
export const DOCS_SECTIONS = [
  ...
  },
  {
    ...
    content: [
      ...
      { type: 'p', text: 'File → New Notebook (⌘N) or click the + button in the tab bar. A dialog lets you choose from a range of topic-based example templates (Getting Started, Data & Charts, Databases, Display & Rich Output, Scripting & Utilities, Workspace & Panels, Cell Orchestration, Forms, Embedded Files, and more) or start blank.' },
  ...
  },
  {
    ...
    content: [
      ...
      { type: 'p', text: 'Click the ▾/▸ arrow at the left of any code cell header to fold the editor down to a single-line preview. The cell remains fully executable while folded — press ▶ Run or Ctrl+Enter to run it without unfolding. When a folded cell has been executed, its elapsed time is shown as a badge on the right of the fold preview. Click the arrow again to expand. Fold state is saved in the .cnb file.' },
      { type: 'h3', text: 'Cell Bookmarks' },
      { type: 'p', text: 'Click the bookmark icon in any cell\'s control bar to bookmark it. Bookmarked cells appear in a dedicated "Bookmarks" section at the top of the Table of Contents panel, providing quick navigation to important cells. Bookmark state is saved in the .cnb file.' },
  ...
  },
  {
    ...
    content: [
      ...
      { type: 'p', text: 'The C# editor connects to a Roslyn LSP server running inside the kernel via a named pipe. Completions, hover quick-info, and real-time semantic diagnostics (type errors, undefined variables, and more) are all powered by Roslyn Workspace APIs. Press Tab to accept a suggestion (Enter inserts a newline as usual). Squiggles appear as you type whenever the kernel is running. Typing "using " triggers namespace completions (System.Net.Http, System.Collections.Concurrent, etc.) by walking the loaded assembly tree.' },
      { type: 'h3', text: 'Hover quick-info' },
      { type: 'p', text: 'Hover over any symbol — a local, parameter, field, property, method, or type — to see its type signature in a tooltip (e.g. hovering nums shows List<int> nums; a method shows its full signature; a type shows class Foo). This is compile-time information from Roslyn, so it works before you run the cell and for symbols that are not variables. It complements the Inline Variable Peek, which additionally shows a variable\'s current runtime value once the cell has run.' },
  ...
  },
  {
    ...
    content: [
      ...
      { type: 'h3', text: 'Large Tables' },
      { type: 'p', text: 'To keep the app responsive, a data table is capped at the first 50,000 rows when it is sent from the kernel to the UI — serialising a huge table as a single message would otherwise stall both the kernel and the app. When a table is capped the panel shows a notice ("Showing the first 50,000 of N rows — output capped by the kernel"). Narrow the result upstream (a WHERE filter, .Take(), or an aggregate in your query/LINQ) to inspect a specific slice.' },
  ...
  },
  {
    ...
    content: [
      ...
      { type: 'ul', items: [
        ...
        'Display.Marquee(text, speed?, color?, background?) — scrolling text ticker; speed is animation duration in seconds',
        'Display.Checklist(title, items…) — pass/fail checklist (green ✓ / red ✗ per item) with a passed/total header; each item is a (label, pass) or (label, pass, note) tuple',
      ]},
      { type: 'code', text: 'Display.StatCard("Revenue", "$1.2M", color: "#4ec9b0", icon: "💰");\nDisplay.ProgressBar(78, "CPU Usage — 78%", color: "#569cd6");\nDisplay.Marquee("  ALERT: All systems operational  ", speed: 30);\n\n// Run checks in code, then show a checklist of what passed\nDisplay.Checklist("Pre-flight",\n    ("Config loaded",   Config.Count > 0),\n    ("Orders present",  orders.Length > 0),\n    ("No negatives",    orders.All(o => o.Price >= 0), "prices must be >= 0"));' },
      { type: 'h3', text: 'Cell Column Layout' },
      { type: 'p', text: 'Set the columns property on consecutive cells to render them side-by-side in a CSS grid. Supported values: 2, 3, 4. Consecutive cells with the same columns value are grouped automatically.' },
  ...
  },
  {
    ...
    content: [
      ...
      { type: 'ul', items: [
        'Geo.GeocodeAsync(query) → GeoResult { Lat, Lon, DisplayName, Country, CountryCode } — Nominatim (OpenStreetMap); cached on disk',
        'Geo.ReverseGeocodeAsync(lat, lon) → GeoResult — Nominatim reverse lookup; cached on disk',
        ...
        'Geo.Distance(a, b) / Geo.Distance(latA, lonA, latB, lonB) → kilometres (Haversine)',
        'Geo.Cluster(points, kmRadius, key) → groups of points within kmRadius of each other (single-link)',
        'Geo.Map(lat, lon, zoom?, markers?, route?, heat?, cluster?, width?, height?, title?) — render an interactive Leaflet map; cluster:true collapses dense markers',
        'Geo.HeatMap(points, centerLat?, centerLon?, zoom?, …) — convenience wrapper, auto-centers on the points',
        'Geo.ClearCache() — wipes the on-disk geocoding cache',
      ]},
      { type: 'h3', text: 'Examples' },
      ...
      { type: 'p', text: 'Every rendered map has a small toolbar in the top-right corner with five buttons: ⊕ fit-to-all-points (re-zooms so every marker, route vertex, and heat point is visible); ↺ reset to the spec\'s original center/zoom; ☼/☾ toggle the basemap between dark and light Carto tiles; ⬇ download the map as a PNG; ⛶ enter or exit native fullscreen.' },
      { type: 'h3', text: 'Geocoding cache' },
      { type: 'p', text: 'Geo.GeocodeAsync and Geo.ReverseGeocodeAsync transparently cache successful results to a JSON file in the user\'s local app data dir. Reruns are instant, work offline after the first run, and never hammer Nominatim. Call Geo.ClearCache() to wipe the cache.' },
  ...
  },
  {
    ...
    content: [
      ...
      { type: 'p', text: 'Expand an attached connection to browse its tables and columns. Each column shows its name, data type, and a PK badge for primary key columns. Click a table header to expand or collapse its column list. A small clipboard badge next to each table name shows the generated C# type name — click it to copy, just like the DbContext variable badge.' },
      { type: 'h3', text: 'Generated Types' },
      { type: 'p', text: 'Attaching generates a POCO class per table, named after the table (e.g. a "Purchases" table → a Purchases class), plus a best-effort EF-style singular alias (Purchase) that refers to the exact same type. Both names resolve at runtime and in IntelliSense — so List<Purchase> and List<Purchases> are interchangeable. The clipboard badge in the schema tree copies the name to use.' },
  ...
  },
  {
    ...
    content: [
      ...
      { type: 'h3', text: 'Memory During Long Sessions' },
      { type: 'p', text: 'The kernel keeps every variable and all compiled state from prior runs so later cells can build on them — this is what makes the notebook feel like one continuous script. Compiled state therefore grows slowly over a long editing session (each executed cell adds a compiled submission), but variable data does not pile up: after every execution the kernel prunes shadowed bindings — the older copies a re-declaration leaves behind — so re-running a cell that loads large data keeps only the current copy alive.' },
      { type: 'p', text: 'Several other things keep memory in check. Per-cell output is capped (if a cell emits thousands of lines or displays, the oldest are coalesced into a single "N earlier outputs hidden to conserve memory" marker), and the Log panel\'s live buffer keeps only the most recent entries. When the kernel\'s memory climbs past a threshold, the status bar shows a red warning; clicking it restarts the kernel (see below). The runtime is also tuned to return freed memory to the OS, so working-set falls back after a cell\'s large intermediate results are garbage-collected.' },
      { type: 'p', text: 'To free a single large variable without restarting, click the eraser (Free) button in its inspector popup, or just assign it null in a cell — e.g. bigData = null; — which drops that variable\'s current value (if nothing else references it). One caveat to pruning: a delegate or lambda captured before a variable was re-declared reads the old copy, and sees null after the re-declaration instead of stale data — re-run the capturing cell (the staleness tracker flags it anyway).' },
      { type: 'h3', text: 'Resetting the Kernel' },
      { type: 'p', text: 'Run → Reset Kernel (or the status-bar memory warning) restarts the kernel: it stops the current kernel process and starts a fresh one, so all accumulated memory — variables, compiled submissions, and loaded assemblies — is fully reclaimed. Cell content and output are preserved on screen; attached DB contexts, NuGet packages, and Config values are re-injected automatically into the fresh kernel. Anything held only in kernel memory is gone, so re-run the cells whose results you still need.' },
  ...
  },
  {
    ...
    content: [
      ...
      { type: 'h3', text: 'Prefix Groups' },
      { type: 'p', text: 'When the Config panel has entries from multiple prefixes (text before the first . or _ in the key), entries are automatically grouped under collapsible headers. Click a group header to expand or collapse it. Entries without a prefix separator are placed in the "other" group.' },
      { type: 'h3', text: 'Import / Export' },
      { type: 'p', text: 'Use the Import button to load entries from a .env file (KEY=value format, one per line; comments and blank lines are skipped). Use Export to save config as a .env file, or JSON to export as structured JSON. Imported entries are added to the existing config (duplicates are not merged automatically).' },
      { type: 'h3', text: 'Scope' },
  ...
  },
  {
    ...
    content: [
      ...
      { type: 'h3', text: 'Copy as C# Literal' },
      { type: 'p', text: 'Click the ⧉ button on any variable row to copy its value as a valid C# literal to the clipboard. The format is type-aware: strings are quoted and escaped, numerics get the correct suffix (L, f, d, m), booleans produce true/false, and null values copy as null.' },
      { type: 'h3', text: 'Variable Diff Highlights' },
      ...
      { type: 'p', text: 'The popup renders the value with the same type inference as the .Display() family: collections show as a table, objects and dictionaries as a collapsible tree, and strings and primitives as text. Popups are persistent and can be dragged anywhere and resized from the bottom-right corner — open as many as you like — and each one re-renders after every cell execution while its variable is in scope, so it stays current even for large collections whose contents change (the value is re-fetched from the kernel, not diffed from a truncated string). The button is disabled for cells whose variables are not yet in scope (run the cell first).' },
      { type: 'p', text: 'Each popup also has an eraser (Free) button next to Close. Clicking it frees that variable — the kernel nulls its binding (and any shadowed older copies) directly on the script state so the objects it held can be garbage-collected — releasing memory without restarting the kernel and without growing the execution chain. Use it for large variables (big collections, DataTables, byte buffers) you are done with; you can always re-run the declaring cell to recompute the value. Free is shown only while the variable is in scope and not already null.' },
    ],
  },
  {
  ...
  },
  {
    ...
    content: [
      ...
      { type: 'h3', text: 'Manual-only cells (never auto-run)' },
      { type: 'p', text: 'A runnable cell can be marked "manual only" from the toggle in its header (the hand icon). A manual-only cell is excluded from every automatic run: it is never pulled in as another cell\'s stale dependency, is skipped by Run All / Run from here / Run to here, dependency traversal does not pass through it, and it does not auto-fire from a schedule, presentation mode, or run-on-startup. It runs only when you click its own ▶ (or Ctrl/Cmd+Enter in it). Use it for cells with side effects — a SQL INSERT/UPDATE, an HTTP POST, a shell command — so running a downstream cell can never silently re-fire them. The header highlights the toggle and the cell gets an amber left rule while the flag is on, and the flag is saved with the notebook. (Explicit Orchestration-panel actions — Run downstream and pipeline runs — are deliberate batch triggers and still include manual-only cells.)' },
      { type: 'h3', text: 'Ambiguous producers + run-plan preview' },
      { type: 'p', text: 'When two or more cells declare the SAME variable, the dependency edge uses the last one in notebook order ("last writer wins"), which can make a surprising cell look like a prerequisite. Such cells now show a warning badge naming the shared variable and the other producers, so the ambiguity is visible rather than mysterious. To see exactly what a run will do before you trigger it, hover the ▶ button: the tooltip lists the cells that will also run (the stale dependencies) and notes any manual-only producers that were skipped.' },
    ],
  },
  {
  ...
  },
  {
    ...
    content: [
      ...
      { type: 'p', text: 'Each match is listed below the test area with its zero-based index, the matched text, and the start–end character positions. Capture groups (numbered and named) are shown indented beneath their parent match. Named groups are visually distinguished with a highlighted badge.' },
      { type: 'h3', text: 'Quick Reference' },
  ...
  },
  {
    ...
    content: [
      ...
      { type: 'ul', items: [
        ...
        'Docker.StopAndRemove(nameOrId) — stop + remove (ignores errors if already stopped)',
        'Docker.StopAllTracked() — stop and remove all containers started by Docker cells this session',
      ...
      { type: 'code', text: '// Start a Redis container for testing\nvar id = Docker.Run("redis:7", name: "test-redis",\n    ports: new() { ["6379"] = "6379" });\n\n// Use it...\nawait Task.Delay(2000); // wait for startup\nDocker.IsRunning("test-redis").Display();\n\n// Clean up\nDocker.StopAndRemove("test-redis");' },
      { type: 'h3', text: 'Mock Server API' },
      { type: 'p', text: 'The Mock global controls mock API servers from code cells. Mock servers run in the main process on random ports above 9000.' },
      { type: 'ul', items: [
        'await Mock.StartAsync(apiDef, port?) — start a mock server, returns the assigned port number',
        'await Mock.StopAsync(id) — stop a mock server by its API definition ID',
        'await Mock.StopAllAsync() — stop all running mock servers',
        'await Mock.ListAsync() — returns list of MockServerInfo (Id, Port, Title)',
      ]},
      { type: 'code', text: '// Start a mock API server\nvar port = await Mock.StartAsync(new {\n    id = "my-api",\n    title = "My API",\n    controllers = new[] { new {\n        basePath = "/api/items",\n        endpoints = new object[] {\n            new { method = "GET", path = "",\n                  mockResponse = new { status = 200, body = "[{\\"id\\":1}]" } },\n        }\n    } }\n});\n\n// Call the mock\nusing var http = new HttpClient();\nvar data = await http.GetStringAsync($"http://localhost:{port}/api/items");\ndata.Display();\n\n// List running mocks\nvar mocks = await Mock.ListAsync();\nmocks.Display();\n\n// Cleanup\nawait Mock.StopAllAsync();' },
  ...
  },
  {
    ...
    content: [
      ...
      { type: 'ul', items: [
        ...
        'Files["name"].ContentCsv — parse as CSV (delimiter auto-detected: comma, semicolon, tab, or pipe)',
        'Files["name"].ContentTsv — parse as TSV (tab)',
