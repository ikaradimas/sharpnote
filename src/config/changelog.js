...
export const CHANGELOG = [
  ...
  { version: '2.30', date: '2026-07-22', title: 'Manual-only cells + ambiguous-producer warnings', gears: 2, items: [
    'Fixed a save freeze on notebooks with embedded files: the version-history sidecar (.cnb.history) was storing a full copy of the notebook — including every embedded file — in all 50 snapshots, so a 6.6MB notebook produced a ~329MB history file that the main process re-read and re-wrote synchronously on every save, freezing the whole app. Snapshots now exclude embedded files and other large blobs (restore only ever used cells/config/title), file writes are async (never block the UI thread), embedded-file notebooks are written compact instead of pretty-printed, and an oversized existing history file is ignored and replaced on the next save (self-healing). Also fixed the 60s auto-backup, which was silently writing empty .bak files (wrong argument) and would otherwise have written synchronously',
  ...
  { version: '2.28', date: '2026-07-22', title: 'Free variables to reclaim memory on demand', gears: 2, items: [
    ...
    'Free / var_release now writes directly to the script state (ScriptVariable.Value) instead of compiling a synthetic assignment — no execution-chain growth per click (~1MB of compilation avoided), shadowed copies are cleared too, and keyword-named variables (var @class) free correctly without escaping',
  ]},
  { version: '2.27', date: '2026-07-22', title: 'Symbol hover, colour-coded ToC, memory bounds, and fixes', gears: 2, items: [
    ...
    'Memory: watch/expression inspection now compiles each expression once and re-runs it for a fresh value, instead of recompiling (and emitting a new assembly) on every refresh — removing a per-refresh assembly leak when a watch re-evaluates after each execution',
    'Memory: the kernel process is GC-tuned to keep its footprint low and return freed memory to the OS (workstation GC, RetainVM disabled, GC.ConserveMemory) — a cell\'s large intermediate results are released back to the OS after collection instead of being hoarded (measured: 755MB of transient allocations fell back to ~180MB)',
    'Memory: when kernel memory climbs past ~1GB the status bar shows a warning you can click to restart the kernel — a fresh process that fully reclaims all accumulated memory. Also documented that "Reset Kernel" restarts the process (full reclaim, re-run needed) and that setting a variable to null frees its current value without a restart',
  ]},
