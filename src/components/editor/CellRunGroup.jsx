...
export function CellRunGroup({ onRun, onInterrupt, onRunFrom, onRunTo, isRunning, disabled, getRunTitle }) {
  ...
  return (
    <div className="cell-run-group" ref={ref}>
      {isRunning ? (
        ...
        <>
          <button className="run-btn" onClick={onRun} disabled={disabled}
                  title="Run (Ctrl+Enter)"
                  onMouseEnter={(e) => { e.currentTarget.title = (getRunTitle && getRunTitle()) || 'Run (Ctrl+Enter)'; }}>
            <Play size={12} /> Run
          </button>
          {(onRunFrom || onRunTo) && (
