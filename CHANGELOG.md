...
## 2.20 ⚙
- Windows build no longer attempts to code-sign or rcedit-edit the executable — `build.win.signAndEditExecutable: false`. Three earlier attempts (`signtoolOptions: null` in 2.20.1, top-level `sign` callback in 2.20.2, nested `signtoolOptions.sign` in 2.20.3) all failed because the no-op sign hook only short-circuits *signing*; electron-builder still downloads `winCodeSign-2.6.0.7z` upfront because it needs `rcedit` from the same archive for the .exe edit pass (icon, version metadata), and the archive's macOS dylib symlinks fail to extract on Windows without admin / Developer Mode. `signAndEditExecutable: false` is the only single switch that bypasses both passes. Trade-off: the SharpNote.exe file itself loses its embedded icon and version metadata; the NSIS installer icon and the Start Menu / Desktop shortcuts still use the configured icon, so the trade-off is invisible to end users in normal use. When a real code-signing identity is acquired later, flip this back and provide actual `signtoolOptions`
