...
## 2.20 ⚙
- Windows build no longer attempts to code-sign without a certificate — `build.win.signtoolOptions.sign` now points at `scripts/sign-noop.js`, a no-op signer that short-circuits the signtool pass and avoids the `winCodeSign-2.6.0.7z` download/extraction that fails on Windows without admin / Developer Mode (the archive contains macOS dylib symlinks Windows can't create). Two earlier attempts (`signtoolOptions: null` in 2.20.1, top-level `sign` in 2.20.2) failed because electron-builder 26.x moved the `sign` callback inside `signtoolOptions` and treats `null` as "use defaults"; the nested form is what the schema accepts
